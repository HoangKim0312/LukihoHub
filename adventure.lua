...
if _AA_Window then
	...
	do
		...
		left:Paragraph({ Title = "LukihoHub", Desc = "Anime Adventures automation. RightControl to toggle." })
		left:Paragraph({ Title = "Version", Desc = _AA_HUB_VERSION })
		left:Button({ Name = "Unload Hub", Callback = function() _AA_unload() end })
		local right = tab:Section({ Side = "Right" })
		right:Header({ Text = "Live" })
		local stateLabel = right:Paragraph({ Title = "State", Desc = "?" })
		local waveLabel  = right:Paragraph({ Title = "Wave",  Desc = "0" })
	...
	do
		...
		left:Paragraph({ Title = "spawn_unit",         Desc = tostring(_AA_get_remote("spawn_unit") ~= nil) })
		left:Paragraph({ Title = "upgrade_unit_ingame", Desc = tostring(_AA_get_remote("upgrade_unit_ingame") ~= nil) })
		left:Paragraph({ Title = "sell_unit_ingame",    Desc = tostring(_AA_get_remote("sell_unit_ingame") ~= nil) })
		left:Paragraph({ Title = "change_priority",     Desc = tostring(_AA_get_remote("change_priority") ~= nil) })
		left:Paragraph({ Title = "use_ingame_spell",    Desc = tostring(_AA_get_remote("use_ingame_spell") ~= nil) })
		left:Paragraph({ Title = "request_join_lobby",  Desc = tostring(_AA_get_remote("request_join_lobby") ~= nil) })
		left:Paragraph({ Title = "use_portal",          Desc = tostring(_AA_get_remote("use_portal") ~= nil) })
		left:Paragraph({ Title = "select_roguelike_option", Desc = tostring(_AA_get_remote("select_roguelike_option") ~= nil) })
		left:Paragraph({ Title = "redeem_code",         Desc = tostring(_AA_get_remote("redeem_code") ~= nil) })
		local right = tab:Section({ Side = "Right" })
